...
template <typename Context>
void AuthAckSessionState<Context>::OnRead(Context& ctx,
...
template <typename Context>
void AuthAckSessionState<Context>::OnWrite(Context& ctx) {
    ctx.DoRead();
}
...
template <typename Context>
void KeyExchangeSessionState<Context>::OnRead(Context& ctx,
                                              const std::string& data) {
...
template <typename Context>
void DataReceiveSessionState<Context>::OnRead(Context& ctx,
                                              const std::string& data) {
    if (data.find("DATA") != std::string::npos) {
        ctx.SetState(tools::make_unique<DataReceiveSessionState<Context>>());
...
template <typename Context>
void DataReceiveSessionState<Context>::OnWrite(Context& ctx) {
    ctx.DoRead();
}
...
template <typename Context>
void FinishSessionState<Context>::OnRead(Context& ctx,
                                         const std::string& /*data*/) {
    ctx.Close();
}
