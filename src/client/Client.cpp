...
FileTransferClient::FileTransferClient(boost::asio::io_context& rtIoContext,
                                       std::string const& rsAddr,
                                       std::string const& rsPort,
                                       std::string const& rsPath)
    : mSocket(rtIoContext, rsAddr, rsPort), mtFilePath(rsPath) {
    mSocket.DoConnect([this]() {
        SetState(tools::make_unique<InitSessionState<FileTransferClient>>());
    });
}

FileTransferClient::~FileTransferClient() { Close(); }
...
void FileTransferClient::SetState(
    std::unique_ptr<SessionState<FileTransferClient>> rpNewState) {
    mpState = std::move(rpNewState);
    mpState->Start(*this);
}

void FileTransferClient::DoRead() {
    mSocket.DoRead(maData, [this](const std::string& buf) {
        if (this->mpState) this->mpState->OnRead(*this, buf);
    });
}

void FileTransferClient::DoWrite(const std::string& raData) {
    mSocket.DoWrite(raData, [this]() {
...
//                                 std::size_t /*length*/) { std::cout <<
//                                 "Message size: " << mdExpectedSize <<"\n"; if
//                                 (!ec && BUF_LEN > mdExpectedSize) {
//                                     DoReadMessage();
//                                 } else {
//                                     std::cout << "Read error: " <<
//                                     ec.message() << "\n";
...
//     boost::asio::async_write(
//         mSocket,
//         boost::asio::buffer(maData,
//         static_cast<size_t>(mtSourceFile.gcount())),
//         [this](boost::system::error_code ec, std::size_t /*length*/) {
//             if (!ec && mtSourceFile.good()) {
//                 SendPacket();
//             } else {
...
//                                   ? "failed"
//                                   : "good";
//                 std::cout << "err value: " << ec << " read status: " <<
//                 status
//                           << "\n";
//                 Close();
//             }
